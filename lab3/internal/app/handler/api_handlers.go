...
type recordResultDTO struct {
	ResultStage *string  `json:"result_stage"`
	ResultMap   *float64 `json:"result_map"`
}

// PUT /api/records/:id/result
// Accept externally computed results (e.g., from async service) and store them in the record.
func (h *Handler) ApiSetRecordResult(c *gin.Context) {
	id, _ := strconv.Atoi(c.Param("id"))
	if id <= 0 {
		jsonFail(c, http.StatusBadRequest, "invalid id")
		return
	}
	var dto recordResultDTO
	if err := c.BindJSON(&dto); err != nil {
		jsonFail(c, http.StatusBadRequest, "invalid json")
		return
	}
	if dto.ResultMap == nil && dto.ResultStage == nil {
		jsonFail(c, http.StatusBadRequest, "empty payload")
		return
	}
	fields := map[string]any{}
	if dto.ResultMap != nil {
		fields["result_map"] = *dto.ResultMap
	}
	if dto.ResultStage != nil {
		fields["result_stage"] = *dto.ResultStage
	}
	if err := h.Repository.UpdateApplicationFields(id, fields); err != nil {
		jsonFail(c, http.StatusNotFound, err.Error())
		return
