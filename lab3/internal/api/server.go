...
func StartServer() {
	...
	// Swagger UI. Keep it on a separate prefix to avoid conflicts with /swagger/openapi.json.
	// Point UI to static /api/swagger.json so it works without generated docs.
	r.GET("/swagger-ui/*any", ginSwagger.WrapHandler(swaggerFiles.Handler, ginSwagger.URL("/api/swagger.json")))
	// Backward-compatible redirects
	r.GET("/swagger", func(c *gin.Context) { c.Redirect(302, "/swagger-ui/index.html") })
	r.GET("/swagger/", func(c *gin.Context) { c.Redirect(302, "/swagger-ui/index.html") })
	r.GET("/swagger/index.html", func(c *gin.Context) { c.Redirect(302, "/swagger-ui/index.html") })
	...
	api.Use(auth.OptionalJWT(jwt))
	{
		...
		api.PUT("/records/:id/result", auth.RequireJWT(jwt), auth.RequireModerator(), handler.ApiSetRecordResult)
	...
	if _, err := os.Stat(swaggerFile); err == nil {
		...
		// Convenience alias: some methodichkas/examples expect this path
		r.StaticFile("/swagger/openapi.json", swaggerFile)
	}
